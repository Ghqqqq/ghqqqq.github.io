---
title: "Enhancing Safety in Reinforcement Learning with Human Feedback via Rectified Policy Optimization"
description: "Selected NeurIPS 2025 paper on safer RLHF and alignment."
venue: "Advances in Neural Information Processing Systems (NeurIPS 2025)"
venueShort: "NeurIPS"
status: "conference"
year: 2025
authors: "Xiyue Peng, Hengquan Guo, Jiawei Zhang, Dongqing Zou, Ziyu Shao, Honghao Wei, Xin Liu"
createdAt: 2025-12-01
selected: true
homepageCategory: "agent-llm-alignment"
lineage:
  label: "Idea lineage"
  text: "Hard constraints → rectified policy optimization"
  sourceTitle: "Online Convex Optimization with Hard Constraints"
  sourceId: "online-convex-optimization-hard-constraints"
---
